...
const getOverlaySnapshot = (): OverlaySnapshot => ({
  protestMarkers: document.querySelectorAll('.event-marker').length,
  ...
  hotspotMarkers: document.querySelectorAll('.trend-zone').length,
});
...
const firstConflictPoint = (fallback: [number, number]): [number, number] => {
  const coords = RISK_REGIONS[0]?.coords?.[0];
...
const [baseLon, baseLat] = firstLatLon(TRACKED_FACILITIES, [44.0, 33.0]);
...
const [hotspotLon, hotspotLat] = firstLatLon(FOCUS_AREAS, [0.0, 20.0]);
const [nuclearLon, nuclearLat] = firstLatLon(ENERGY_FACILITIES, [14.0, 50.0]);
const [irradiatorLon, irradiatorLat] = firstLatLon(GAMMA_IRRADIATORS, [12.0, 50.0]);
const [waterwayLon, waterwayLat] = firstLatLon(KEY_WATERWAYS, [32.0, 30.0]);
...
const VISUAL_SCENARIOS: VisualScenario[] = [
  {
    ...
    expectedDeckLayers: ['risk-zones-layer'],
    expectedSelectors: [],
  },
  {
    id: 'bases-z5',
  ...
  },
  {
    ...
    expectedDeckLayers: ['trends-layer'],
    expectedSelectors: [],
  },
  {
    id: 'ais-z5',
  ...
  },
  {
    ...
    expectedDeckLayers: ['facility-layer'],
    expectedSelectors: [],
  },
  {
    id: 'irradiators-z5',
    ...
    expectedDeckLayers: ['nodes-layer'],
    expectedSelectors: [],
  },
  {
    id: 'weather-z5',
  ...
  },
  {
    ...
    expectedDeckLayers: [],
    expectedSelectors: ['.event-marker'],
  },
  {
    id: 'flights-z5',
    variant: 'both',
...
const seedAllDynamicData = (): void => {
  ...
  const trackedFlights: TrackedFlight[] = [
    {
      id: 'e2e-mil-flight-1',
      callsign: 'HARN01',
      hexCode: 'abc123',
  ...
  const trackedFlightClusters: FlightCluster[] = [
    {
  ...
  const trackedVesselClusters: VesselCluster[] = [
    {
  ...
  map.setSecurityThreats(securityThreats);
  ...
  map.setTrackedFlights(trackedFlights, trackedFlightClusters);
  map.setTrackedVessels(trackedVessels, trackedVesselClusters);
