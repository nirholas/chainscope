...
const SOURCE_METADATA: Record<DataSourceId, { name: string; requiredForRisk: boolean; panelId?: string }> = {
  acled: { name: 'Governance & Proposals', requiredForRisk: false, panelId: 'protests' },
  opensky: { name: 'Whale Tracking', requiredForRisk: false, panelId: 'military' },
  wingbits: { name: 'Wallet Enrichment', requiredForRisk: false, panelId: 'military' },
  ais: { name: 'DEX Volume', requiredForRisk: false, panelId: 'shipping' },
  usgs: { name: 'Market Events', requiredForRisk: false, panelId: 'natural' },
  gdelt: { name: 'Crypto News', requiredForRisk: true, panelId: 'intel' },
  ...
  firms: { name: 'On-chain Anomalies', requiredForRisk: false, panelId: 'map' },
  acled_conflict: { name: 'Protocol Exploits', requiredForRisk: false, panelId: 'protests' },
  ucdp: { name: 'Exploit Classification', requiredForRisk: false, panelId: 'protests' },
  hapi: { name: 'DeFi Aggregates', requiredForRisk: false, panelId: 'protests' },
  ucdp_events: { name: 'Rug Pull Events', requiredForRisk: false, panelId: 'ucdp-events' },
  unhcr: { name: 'Capital Flows', requiredForRisk: false, panelId: 'displacement' },
...
// Data gap messages - explains what's NOT visible (Quick Win #1)
const DATA_GAP_MESSAGES: Record<DataSourceId, string> = {
  acled: 'Governance/proposal events may be missed—DAO data unavailable',
  opensky: 'Whale wallet positions unknown—on-chain tracking offline',
  wingbits: 'Wallet identification limited—enrichment service unavailable',
  ais: 'DEX positions outdated—possible wash trading or dark pool activity undetected',
  usgs: 'Recent market events may not be shown—data unavailable',
  gdelt: 'Crypto news velocity unknown—market intel feed offline',
  rss: 'Breaking DeFi news may be missed—RSS feeds not updating',
  polymarket: 'Prediction market signals unavailable—early warning capability degraded',
  outages: 'Network disruptions may be unreported—RPC monitoring offline',
  cyber_threats: 'Cyber IOC map points unavailable—exploit visibility reduced',
  weather: 'Market sentiment warnings may be missed—sentiment data unavailable',
  economic: 'Economic indicators stale—macro data not updating',
  oil: 'Energy market analytics unavailable—data not updating',
  spending: 'Treasury data unavailable',
  firms: 'On-chain anomaly detection unavailable—data not updating',
  acled_conflict: 'Protocol exploit events may be missed—security data unavailable',
  ucdp: 'Exploit classification unavailable—data not loading',
  hapi: 'Aggregated DeFi data unavailable—analytics not responding',
  ucdp_events: 'Rug pull event-level data unavailable',
  unhcr: 'Capital flow data unavailable—fund movements unknown',
...
/**
 * Get data gap warnings for stale or unavailable data sources.
 * These warnings help users understand what they CANNOT see.
 */
export function getDataGaps(): { source: DataSourceId; message: string; severity: 'warning' | 'critical' }[] {
  ...
  for (const source of dataFreshness.getAllSources()) {
    if (source.status === 'no_data' || source.status === 'very_stale' || source.status === 'error') {
      const message = DATA_GAP_MESSAGES[source.id] || `${source.name} data unavailable`;
...
/**
 * Get a formatted data gap summary for display.
 */
export function getDataGapSummary(): string[] {
...
/**
 * Check if there are any critical data gaps.
 */
export function hasCriticalGaps(): boolean {
  return getDataGaps().some(gap => gap.severity === 'critical');
}
