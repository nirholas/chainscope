const DEFAULT_REMOTE_HOSTS: Record<string, string> = {
  tech: 'https://defistech.vercel.app',
  full: 'https://defistech.vercel.app',
  world: 'https://defistech.vercel.app',
};
...
export function getRemoteApiBaseUrl(): string {
  ...
  return DEFAULT_REMOTE_HOSTS[variant] ?? DEFAULT_REMOTE_HOSTS.full ?? 'https://defistech.vercel.app';
}
...
function isAppOriginUrl(urlStr: string): boolean {
  try {
    ...
    return APP_HOSTS.has(host) || host.endsWith('.defis.tech') || host.endsWith('.vercel.app');
  } catch {
    return false;
  }
}
