import type { SocialUnrestEvent, TrackedFlight, TrackedVessel, ClusteredEvent, InternetOutage } from '@/types';
import { FOCUS_AREAS, RISK_REGIONS, KEY_WATERWAYS } from '@/config/geo';
...
function initCountryData(): CountryData {
  return { protests: [], conflicts: [], ucdpStatus: null, hapiSummary: null, trackedFlights: [], trackedVessels: [], newsEvents: [], outages: [], displacementOutflow: 0, climateStress: 0 };
}
...
export function ingestConflicts(events: ConflictEvent[]): void {
  for (const e of events) {
    const code = normalizeCountryName(e.country);
    if (!code || !TIER1_COUNTRIES[code]) continue;
    if (!countryDataMap.has(code)) countryDataMap.set(code, initCountryData());
...
export function ingestUcdp(classifications: Map<string, UcdpConflictStatus>): void {
...
export function ingestHapi(summaries: Map<string, HapiConflictSummary>): void {
...
export function ingestDisplacement(countries: CountryDisplacement[]): void {
...
export function ingestClimate(anomalies: ClimateAnomaly[]): void {
  for (const data of countryDataMap.values()) {
...
function trackHotspotActivity(lat: number, lon: number, weight: number = 1): void {
  ...
  for (const zone of RISK_REGIONS) {
  ...
  for (const waterway of KEY_WATERWAYS) {
...
export function ingestActivityData(flights: TrackedFlight[], vessels: TrackedVessel[]): void {
  // Track foreign activity per country
  const foreignActivityByCountry = new Map<string, { flights: number; vessels: number }>();
  ...
  for (const f of flights) {
    ...
    if (operatorCode && TIER1_COUNTRIES[operatorCode]) {
      ...
      countryDataMap.get(operatorCode)!.trackedFlights.push(f);
    }
    ...
    if (locationCode && TIER1_COUNTRIES[locationCode] && locationCode !== operatorCode) {
      if (!foreignActivityByCountry.has(locationCode)) {
        foreignActivityByCountry.set(locationCode, { flights: 0, vessels: 0 });
      }
      foreignActivityByCountry.get(locationCode)!.flights++;
    }
  ...
  for (const v of vessels) {
    ...
    if (operatorCode && TIER1_COUNTRIES[operatorCode]) {
      if (!countryDataMap.has(operatorCode)) countryDataMap.set(operatorCode, initCountryData());
      countryDataMap.get(operatorCode)!.trackedVessels.push(v);
    }

    // 2. Credit LOCATION country if different (foreign presence)
    const locationCode = getCountryFromLocation(v.lat, v.lon);
    if (locationCode && TIER1_COUNTRIES[locationCode] && locationCode !== operatorCode) {
      if (!foreignActivityByCountry.has(locationCode)) {
        foreignActivityByCountry.set(locationCode, { flights: 0, vessels: 0 });
      }
      foreignActivityByCountry.get(locationCode)!.vessels++;
    }
    trackHotspotActivity(v.lat, v.lon, 2);
  }

  // Store foreign activity counts for security calculation
  for (const [code, counts] of foreignActivityByCountry) {
    if (!countryDataMap.has(code)) countryDataMap.set(code, initCountryData());
    const data = countryDataMap.get(code)!;
    // Add synthetic entries to represent foreign presence
    // Each foreign flight/vessel counts MORE than own (elevated risk)
    for (let i = 0; i < counts.flights * 2; i++) {
      data.trackedFlights.push({} as TrackedFlight);
    }
    for (let i = 0; i < counts.vessels * 2; i++) {
      data.trackedVessels.push({} as TrackedVessel);
    }
  }
}

export function ingestNews(events: ClusteredEvent[]): void {
  for (const e of events) {
...
export function ingestOutages(outages: InternetOutage[]): void {
...
function calcSecurityScore(data: CountryData): number {
  const flights = data.trackedFlights.length;
  const vessels = data.trackedVessels.length;
...
export function calculateRiskScores(): CountryScore[] {
...
export function getTopUnstableCountries(limit = 10): CountryScore[] {
  return calculateRiskScores().slice(0, limit);
}
