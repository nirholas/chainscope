/**
 * Focal Point Detector - Cross-Signal Synthesis Layer
 *
 * Correlates news entities with map signals to identify "main characters"
 * that appear across multiple data streams.
 *
 * Example: IRAN mentioned in 12 news clusters + 5 tracked flights + internet outage
...
const SIGNAL_TYPE_LABELS: Record<SignalType, string> = {
  ...
  ais_disruption: 'signal disruption',
...
class FocalPointDetector {
  ...
  private calculateCorrelationBonus(
    ...
    if (signals && mention.topHeadlines.some(h => {
      ...
      return (signals.signalTypes.has('tracked_flight') && /military|troops|forces|army|air force/.test(lower)) ||
             (signals.signalTypes.has('tracked_vessel') && /navy|naval|ships|fleet|carrier/.test(lower)) ||
  ...
  private generateAIContext(focalPoints: FocalPoint[]): string {
    ...
    const lines: string[] = ['[MARKET SYNTHESIS]'];
