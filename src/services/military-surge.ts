import type { TrackedFlight, FleetOperator } from '@/types';
...
function isTransportFlight(flight: TrackedFlight): boolean {
...
function classifyFlight(flight: TrackedFlight): 'transport' | 'fighter' | 'recon' | 'other' {
...
function getTheaterForFlight(flight: TrackedFlight): MilitaryTheater | null {
...
export function analyzeFlightsForSurge(flights: TrackedFlight[]): SurgeAlert[] {
  cleanupOldHistory();

  const theaterFlights = new Map<string, TrackedFlight[]>();
...
function isHomeRegion(operator: FleetOperator, regionId: string): boolean {
...
function getOperatorThreshold(operator: FleetOperator): number {
  const config = OPERATOR_HOMES.find(o => o.operator === operator);
  return config?.alertThreshold ?? 3;
}

function getOperatorCountry(operator: FleetOperator): string {
  const config = OPERATOR_HOMES.find(o => o.operator === operator);
  return config?.country ?? 'Unknown';
}

export function detectForeignPresence(flights: TrackedFlight[]): ForeignPresenceAlert[] {
  const newAlerts: ForeignPresenceAlert[] = [];

  // Group flights by operator and region
  const presenceMap = new Map<string, { operator: FleetOperator; region: GeoRegion; flights: TrackedFlight[] }>();
...
export function foreignPresenceToSignal(alert: ForeignPresenceAlert): {
  ...
  return {
    ...
    title: `🚨 ${alert.operatorCountry} Whales in ${alert.region.name}`,
    description,
    severity,
    confidence,
    category: 'military',
...
export function surgeAlertToSignal(surge: SurgeAlert): {
  ...
  const typeLabels = {
    airlift: '🛋 Large Volume Surge',
    fighter: '📈 High Frequency Surge',
    reconnaissance: '🔍 Scan Activity Surge',
  };
  ...
  return {
    ...
    description: `${surge.currentCount} ${surge.type} transactions detected (${surge.surgeMultiple.toFixed(1)}x baseline). ` +
...
export function getTheaterPostureSummaries(flights: TrackedFlight[]): TheaterPostureSummary[] {
  ...
  for (const theater of POSTURE_THEATERS) {
    ...
    const parts: string[] = [];
    if (byType.fighters > 0) parts.push(`${byType.fighters} swaps`);
    if (byType.tankers > 0) parts.push(`${byType.tankers} liquidity events`);
    if (byType.awacs > 0) parts.push(`${byType.awacs} oracle updates`);
    if (byType.reconnaissance > 0) parts.push(`${byType.reconnaissance} scans`);
    const summary = parts.join(', ') || 'No whale transactions';

    const headline =
      postureLevel === 'critical'
        ? `Critical whale accumulation - ${theater.name}`
        : postureLevel === 'elevated'
          ? `Elevated whale activity - ${theater.name}`
...
export function recalcPostureWithVessels(postures: TheaterPostureSummary[]): void {
  for (const p of postures) {
    ...
    const parts: string[] = [];
    if (p.totalAircraft > 0) parts.push(`${p.totalAircraft} transactions`);
    if (p.totalVessels > 0) parts.push(`${p.totalVessels} protocols`);
    const assetSummary = parts.join(' + ') || 'No assets';

    p.headline =
      p.postureLevel === 'critical'
        ? `Critical whale accumulation - ${p.theaterName} (${assetSummary})`
        : p.postureLevel === 'elevated'
          ? `Elevated whale activity - ${p.theaterName} (${assetSummary})`
          : `Normal activity - ${p.theaterName}`;
  }
}

export function getCriticalPostures(flights: TrackedFlight[]): TheaterPostureSummary[] {
