import type { TrackedVessel, VesselCluster, VesselCategory, FleetOperator } from '@/types';
...
let vesselCache: { data: TrackedVessel[]; timestamp: number } | null = null;

// In-memory vessel tracking
const trackedVessels = new Map<string, TrackedVessel>();
...
// Circuit breaker
const breaker = createCircuitBreaker<{ vessels: TrackedVessel[]; clusters: VesselCluster[] }>({
  name: 'Protocol Signal Tracking',
  maxFailures: 3,
 ...
 */
function getVesselTypeFromAis(shipType: number): VesselCategory | undefined {
  // AIS ship type codes
  // 35 = Military ops
  // 50-59 = Special craft
  // 55 = Law enforcement
...
function processAisPosition(data: AisPositionData): void {
  ...
  // Determine operator
  let operator: FleetOperator | 'other' = 'other';
  ...
  if (previousSize === 0 || (trackedVessels.size === 10 && previousSize < 10)) {
    vesselCache = null;
    breaker.clearCache();
    console.log(`[Vessel Tracking] Cleared caches - first vessels arriving (count: ${trackedVessels.size})`);
  }

  if (messageCount % 50 === 0) {
    console.log(`[Vessel Tracking] Tracking ${trackedVessels.size} tracked vessels`);
  }
}
 ...
 */
function clusterVessels(vessels: TrackedVessel[]): VesselCluster[] {
  ...
  for (const hotspot of MILITARY_HOTSPOTS) {
    ...
    if (nearbyVessels.length >= 2) {
      ...
      clusters.push({
        ...
        name: `${hotspot.name} Protocol Activity`,
        lat: avgLat,
        lon: avgLon,
...
// Initialize cleanup interval with proper lifecycle management
let cleanupIntervalId: ReturnType<typeof setInterval> | null = null;

export function startVesselHistoryCleanup(): void {
  if (cleanupIntervalId !== null) return;
  cleanupIntervalId = setInterval(cleanup, HISTORY_CLEANUP_INTERVAL);
}
 ...
 */
export function initVesselStream(): void {
  if (isTracking) return;

  console.log('[Vessel Tracking] Initializing tracking via shared AIS stream...');
 ...
 */
export function getVesselTrackingStatus(): { connected: boolean; vessels: number; messages: number } {
  return {
 ...
 */
export async function fetchTrackedVessels(): Promise<{
  vessels: TrackedVessel[];
  clusters: VesselCluster[];
}> {
  // Debug: check state before calling breaker
  console.log(`[Vessel Tracking] fetchTrackedVessels called - isTracking: ${isTracking}, isAisConfigured: ${isAisConfigured()}, trackedVessels.size: ${trackedVessels.size}`);

  return breaker.execute(async () => {
    // Check cache first
    if (vesselCache && Date.now() - vesselCache.timestamp < CACHE_TTL) {
      console.log(`[Vessel Tracking] Returning cached: ${vesselCache.data.length} vessels`);
    ...
    if (!isTracking && isAisConfigured()) {
      console.log('[Vessel Tracking] Initializing stream from fetchTrackedVessels...');
      initVesselStream();
    }
    ...
    console.log(`[Vessel Tracking] After cleanup, returning ${vessels.length} vessels (trackedVessels.size: ${trackedVessels.size})`);
 ...
 */
export function getVesselsCircuitStatus(): string {
 ...
 * Get vessel by MMSI
 */
export function getVesselByMmsi(mmsi: string): TrackedVessel | undefined {
 ...
 */
export function getVesselsNearLocation(lat: number, lon: number, radiusDeg: number = 2): TrackedVessel[] {
  const result: TrackedVessel[] = [];
 ...
 */
export function getDarkVessels(): TrackedVessel[] {
 ...
 */
export function isVesselTrackingConfigured(): boolean {
