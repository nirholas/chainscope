import type { SocialUnrestEvent, TrackedFlight, TrackedVessel, Earthquake } from '@/types';
...
export type GeoEventType = 'protest' | 'tracked_flight' | 'tracked_vessel' | 'earthquake';
...
export function ingestFlights(flights: TrackedFlight[]): void {
  for (const f of flights) {
    ingestGeoEvent(f.lat, f.lon, 'tracked_flight', f.lastSeen);
  }
}

export function ingestVessels(vessels: TrackedVessel[]): void {
  for (const v of vessels) {
    ingestGeoEvent(v.lat, v.lon, 'tracked_vessel', v.lastAisUpdate);
  }
}
...
const TYPE_LABELS: Record<GeoEventType, string> = {
  ...
  tracked_flight: 'whale transactions',
  tracked_vessel: 'protocol exploits',
  earthquake: 'market events',
};
...
export function debugInjectTestEvents(): void {
  ...
  ingestGeoEvent(testLat, testLon, 'tracked_flight', now);
  ingestGeoEvent(testLat, testLon, 'tracked_vessel', now);
...
export function debugGetCells(): Map<string, unknown> | null {
