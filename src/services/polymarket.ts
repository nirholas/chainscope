...
// Default to false — Cloudflare consistently blocks non-browser origins (e.g. Vercel preview deploys)
let directFetchWorks: boolean | null = false;
...
async function polyFetch(endpoint: 'events' | 'markets', params: Record<string, string>): Promise<Response> {
  ...
  // Only attempt on localhost where CORS isn't an issue
  if (directFetchWorks !== false && /^https?:\/\/(localhost|127\.0\.0\.1)/.test(location.origin)) {
    try {
  ...
  return fetch(`https://defistech.vercel.app/api/polymarket?${proxyQs}`);
}
