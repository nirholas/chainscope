...
function humanizeSignalType(type: string): string {
  const map: Record<string, string> = {
    ...
    military_surge: 'Whale Surge',
    military_flight: 'Whale Transactions',
    internet_outage: 'Internet Outages',
    protest: 'Governance Proposals',
    naval_vessel: 'Protocol Exploits',
    ais_gap: 'Signal Gaps',
    satellite_fire: 'On-chain Anomalies',
  };
...
const LOGO_URL = '/favico/defis-icon-1024.png';
...
export async function renderStoryToCanvas(data: StoryData): Promise<HTMLCanvasElement> {
  ...
  ctx.fillText('CHAINSCOPE', textX, y + 26);
  ...
  if (data.cii?.components) {
    y += 44;
    const comps = [
      { label: 'GOVERNANCE', val: data.cii.components.unrest, color: '#f97316' },
      { label: 'EXPLOITS', val: data.cii.components.conflict, color: '#dc2626' },
      { label: 'SECURITY', val: data.cii.components.security, color: '#ef4444' },
      { label: 'SENTIMENT', val: data.cii.components.information, color: '#8b5cf6' },
    ];
  ...
  // ── ACTIVE SIGNALS ──
  const hasSignals = data.signals.protests + data.signals.trackedFlights + data.signals.trackedVessels + data.signals.outages > 0;
  if (hasSignals) {
    y += 40;
    drawSeparator(ctx, y, PAD);
    y += 46;
    ...
    y += 48;
    const sigItems = [
      { icon: '📢', label: 'Governance', count: data.signals.protests, color: '#f97316' },
      { icon: '💸', label: 'Whale Transactions', count: data.signals.trackedFlights, color: '#ef4444' },
      { icon: '🔗', label: 'Protocol Exploits', count: data.signals.trackedVessels, color: '#3b82f6' },
      { icon: '🌐', label: 'Network Outages', count: data.signals.outages, color: '#8b5cf6' },
    ].filter(s => s.count > 0);
  ...
  if (data.theater && y < FOOTER_Y - 200) {
    ...
    drawSeparator(ctx, y, PAD);
    y += 46;
    drawSectionHeader(ctx, 'DEFI RISK POSTURE', PAD, y);
    ...
    ctx.fillStyle = '#bbb';
    ctx.fillText(`💸 ${data.theater.totalAircraft} whale txns`, PAD, y);
    const acW = ctx.measureText(`💸 ${data.theater.totalAircraft} whale txns`).width;
    ctx.fillText(`🔗 ${data.theater.totalVessels} exploits`, PAD + acW + 40, y);
    ...
    if (data.theater.fighters || data.theater.tankers || data.theater.awacs) {
      ...
      ctx.font = '400 24px Inter, system-ui, sans-serif';
      const parts: string[] = [];
      if (data.theater.fighters) parts.push(`Swaps: ${data.theater.fighters}`);
      if (data.theater.tankers) parts.push(`Liquidity: ${data.theater.tankers}`);
      if (data.theater.awacs) parts.push(`Oracles: ${data.theater.awacs}`);
  ...
  ctx.fillText('CHAINSCOPE', footerTextX, H - 55);
  ctx.letterSpacing = '0px';
  ctx.font = '400 20px Inter, system-ui, sans-serif';
  ctx.fillText('DeFi Operating System', footerTextX, H - 30);
