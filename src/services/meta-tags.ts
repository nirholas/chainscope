// Dynamic Meta Tags Service for Chainscope
...
const BASE_URL = 'https://defistech.vercel.app';
const DEFAULT_IMAGE = 'https://defistech.vercel.app/favico/og-image.png';
...
export function updateMetaTagsForStory(meta: StoryMeta): void {
  ...
  const title = `${countryName} DeFi Brief | Chainscope`;
...
export function resetMetaTags(): void {
  const defaultTitle = 'Chainscope - DeFi Operating System';
  const defaultDesc = 'AI-powered real-time DeFi and crypto intelligence dashboard with live news, markets, on-chain tracking, and protocol analytics.';
...
function generateDescription(
  ...
  if (score !== undefined && level) {
    parts.push(`${countryName} has a risk score of ${score}/100 (${level})`);
  }
  ...
  const typeDescriptions: Record<string, string> = {
    ciianalysis: 'Full DeFi analysis with protocol risk and prediction markets',
