...
class SignalAggregator {
  ...
  ingestFlights(flights: TrackedFlight[]): void {
    this.clearSignalType('tracked_flight');
    ...
    for (const [code, count] of countryCounts) {
      this.signals.push({
        ...
        title: `${count} whale transactions detected`,
        timestamp: new Date(),
      });
    }
    this.pruneOld();
  ...
  ingestVessels(vessels: TrackedVessel[]): void {
    this.clearSignalType('tracked_vessel');
    ...
    for (const [code, data] of regionCounts) {
      this.signals.push({
        ...
        title: `${data.count} protocol exploits near region`,
        timestamp: new Date(),
      });
    }
    this.pruneOld();
  ...
  ingestProtests(events: SocialUnrestEvent[]): void {
    ...
    for (const [code, data] of countryCounts) {
      this.signals.push({
        ...
        title: `${data.count} governance events`,
        timestamp: new Date(),
      });
    }
    this.pruneOld();
    ...
    for (const [_regionId, def] of Object.entries(REGION_DEFINITIONS)) {
      ...
      if (allTypes.size >= 2) {
        const typeLabels: Record<SignalType, string> = {
          ...
          tracked_flight: 'whale transaction activity',
          tracked_vessel: 'protocol exploit activity',
          protest: 'governance proposals',
          ais_disruption: 'signal anomalies',
