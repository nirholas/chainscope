...
if (stored) {
  ...
  console.log('[CachedPosture] Restored from localStorage (stale)');
}
...
export async function fetchCachedTheaterPosture(): Promise<CachedTheaterPosture | null> {
  ...
  fetchPromise = (async () => {
    try {
      ...
      if (!response.ok) {
        console.warn('[CachedPosture] API error:', response.status);
