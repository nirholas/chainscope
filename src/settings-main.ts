...
function initPreferences(): void {
  const toggle = document.getElementById('showProductivityWidgets') as HTMLInputElement | null;
  if (!toggle) return;

  // Reflect current stored value
  toggle.checked = localStorage.getItem('chainscope-show-productivity') === 'true';

  toggle.addEventListener('change', () => {
    if (toggle.checked) {
      localStorage.setItem('chainscope-show-productivity', 'true');
    } else {
      localStorage.removeItem('chainscope-show-productivity');
    }
    setActionStatus('Productivity widgets ' + (toggle.checked ? 'enabled' : 'disabled') + ' — refresh the main page to apply', 'ok');
  });
}
