...
async function renderAndDisplay(data: StoryData): Promise<void> {
  ...
  if (content) {
    ...
    img.alt = `${data.countryName} DeFi Story`;
...
function downloadStory(): void {
  ...
  a.download = `chainscope-${currentData?.countryCode.toLowerCase() || 'story'}-${Date.now()}.png`;
  a.click();
...
async function shareWhatsApp(data: StoryData): Promise<void> {
  ...
  const file = new File([currentBlob], `${data.countryCode.toLowerCase()}-chainscope.png`, { type: 'image/png' });
