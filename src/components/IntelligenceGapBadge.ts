...
export class IntelligenceFindingsBadge {
  ...
  constructor() {
    ...
    this.badge.className = 'analysis-findings-badge';
    this.badge.title = 'DeFi findings';
    this.badge.innerHTML = '<span class="findings-icon">🎯</span><span class="findings-count">0</span>';

    this.dropdown = document.createElement('div');
    this.dropdown.className = 'analysis-findings-dropdown';
  ...
  public update(): void {
    ...
    if (count === 0) {
      ...
      this.badge.title = 'No recent DeFi findings';
    } else if (hasCritical || hasHigh) {
      this.badge.classList.add('status-high');
      this.badge.title = `${count} DeFi findings - review recommended`;
    } else if (count <= LOW_COUNT_THRESHOLD) {
      this.badge.classList.add('status-low');
      this.badge.title = `${count} DeFi finding${count > 1 ? 's' : ''}`;
    } else {
      this.badge.classList.add('status-high');
      this.badge.title = `${count} DeFi findings - review recommended`;
    }
    ...
    if (this.findings.length === 0) {
      this.dropdown.innerHTML = `
        <div class="findings-header">
          <span class="header-title">DeFi Findings</span>
    ...
    overlay.innerHTML = `
      <div class="findings-modal">
        <div class="findings-modal-header">
          <span class="findings-modal-title">🎯 All DeFi Findings (${this.findings.length})</span>
