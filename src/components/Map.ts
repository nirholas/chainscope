...
import type { MapLayers, Hotspot, NewsItem, Earthquake, InternetOutage, RelatedAsset, AssetType, AisDisruptionEvent, AisDensityZone, CableAdvisory, RepairShip, SocialUnrestEvent, AirportDelayAlert, TrackedFlight, TrackedVessel, FlightCluster, VesselCluster, NaturalEvent, SecurityThreat } from '@/types';
...
export class MapComponent {
    ...
    const fullHelpContent = `
      ...
      <div class="layer-help-content">
        <div class="layer-help-section">
          ...
          <div class="layer-help-note">Affects: Market Events, Sentiment, Governance, Outages</div>
        </div>
        <div class="layer-help-section">
  ...
  private renderConflicts(projection: d3.GeoProjection): void {
    if (!this.dynamicLayerGroup) return;
    const conflictGroup = this.dynamicLayerGroup.append('g').attr('class', 'risks');

    RISK_REGIONS.forEach((zone) => {
  ...
  private updateCountryFills(): void {
    ...
    this.baseLayerGroup.selectAll('.country').each(function (datum) {
      ...
      if (id?.id !== undefined && FLAGGED_COUNTRIES[id.id]) {
        const level = FLAGGED_COUNTRIES[id.id];
        if (level) {
  ...
  private renderOverlays(projection: d3.GeoProjection): void {
    ...
    if (this.state.layers.nuclear) {
      ENERGY_FACILITIES.forEach((facility) => {
        const pos = projection([facility.lon, facility.lat]);
        if (!pos) return;

        const div = document.createElement('div');
        const isHighlighted = this.highlightedAssets.energy.has(facility.id);
        div.className = `facility-marker ${facility.status}${isHighlighted ? ' asset-highlight asset-highlight-nuclear' : ''}`;
    ...
    if (this.state.layers.conflicts) {
      RISK_REGIONS.forEach((zone) => {
        ...
        clickArea.className = 'risk-click-area';
    ...
    if (this.state.layers.hotspots) {
      this.hotspots.forEach((spot) => {
        ...
        const div = document.createElement('div');
        div.className = 'trend-zone';
        div.style.left = `${pos[0]}px`;
        div.style.top = `${pos[1]}px`;

        div.innerHTML = `
          <div class="trend-marker ${escapeHtml(spot.level || 'low')}"></div>
        `;
    ...
    // Tracked facilities (always HTML - nation colors matter)
    if (this.state.layers.bases) {
      TRACKED_FACILITIES.forEach((base) => {
        const pos = projection([base.lon, base.lat]);
        if (!pos) return;

        const div = document.createElement('div');
        const isHighlighted = this.highlightedAssets.facility.has(base.id);
        div.className = `hub-marker ${base.type}${isHighlighted ? ' asset-highlight asset-highlight-base' : ''}`;
        div.style.left = `${pos[0]}px`;
        div.style.top = `${pos[1]}px`;
    ...
    if (this.state.layers.protests) {
      ...
      clusters.forEach((cluster) => {
        ...
        div.className = `event-marker ${hasHighSeverity ? 'high' : primaryEvent.severity} ${hasRiot ? 'riot' : primaryEvent.eventType} ${isCluster ? 'cluster' : ''}`;
        div.style.left = `${cluster.pos[0]}px`;
        div.style.top = `${cluster.pos[1]}px`;
    ...
    // Fleet Tracking (flights and vessels)
    if (this.state.layers.military) {
      ...
      this.trackedFlights.forEach((flight) => {
        ...
        const div = document.createElement('div');
        div.className = `tracker-marker ${flight.operator} ${flight.aircraftType}${flight.isInteresting ? ' interesting' : ''}`;
        div.style.left = `${pos[0]}px`;
        div.style.top = `${pos[1]}px`;

        // Crosshair icon - rotates with heading
        const icon = document.createElement('div');
        icon.className = `tracker-icon ${flight.aircraftType}`;
        ...
        if (flight.altitude > 0) {
          ...
          alt.className = 'tracker-altitude';
        ...
        if (flight.track && flight.track.length > 1 && this.state.zoom >= 2) {
          ...
          if (points) {
            ...
            trackLine.setAttribute('class', `tracker-track ${flight.operator}`);
      ...
      this.trackedFlightClusters.forEach((cluster) => {
        const pos = projection([cluster.lon, cluster.lat]);
        if (!pos) return;

        const div = document.createElement('div');
        div.className = `activity-cluster tracker-cluster ${cluster.activityType || 'unknown'}`;
        div.style.left = `${pos[0]}px`;
        div.style.top = `${pos[1]}px`;
      ...
      // Tracked Vessels (warships, carriers, submarines)
      ...
      this.trackedVessels.forEach((vessel) => {
        ...
        const div = document.createElement('div');
        div.className = `route-marker ${vessel.operator} ${vessel.vesselType}${vessel.isDark ? ' dark-vessel' : ''}${vessel.isInteresting ? ' interesting' : ''}`;
        div.style.left = `${pos[0]}px`;
        div.style.top = `${pos[1]}px`;

        const icon = document.createElement('div');
        icon.className = `route-icon ${vessel.vesselType}`;
        ...
        if (vessel.isDark) {
          ...
          darkIndicator.title = 'Protocol Signal Lost';
        ...
        if (vessel.track && vessel.track.length > 1 && this.state.zoom >= 2) {
          ...
          if (points) {
            trackLine.setAttribute('points', points);
            trackLine.setAttribute('class', `route-track ${vessel.operator}`);
            trackLine.setAttribute('fill', 'none');
      ...
      this.trackedVesselClusters.forEach((cluster) => {
        const pos = projection([cluster.lon, cluster.lat]);
        if (!pos) return;

        const div = document.createElement('div');
        div.className = `activity-cluster route-cluster ${cluster.activityType || 'unknown'}`;
        div.style.left = `${pos[0]}px`;
        div.style.top = `${pos[1]}px`;
  ...
  private renderWaterways(projection: d3.GeoProjection): void {
    KEY_WATERWAYS.forEach((waterway) => {
  ...
  private updateLabelVisibility(zoom: number): void {
    const labels = this.overlays.querySelectorAll('.trend-label, .earthquake-label, .weather-label, .apt-label');
    const labelRects: { el: Element; rect: DOMRect; priority: number }[] = [];

    // Collect all label bounds with priority
    labels.forEach((label) => {
      const el = label as HTMLElement;
      const parent = el.closest('.trend-zone, .earthquake-marker, .weather-marker, .apt-marker');

      // Assign priority based on parent type and level
      let priority = 1;
      if (parent?.classList.contains('trend-zone')) {
        const marker = parent.querySelector('.trend-marker');
  ...
  public setSecurityThreats(_threats: SecurityThreat[]): void {
    // SVG/mobile fallback intentionally does not render this layer to stay lightweight.
  }
  /** @deprecated Use setSecurityThreats */
  public setCyberThreats(_threats: SecurityThreat[]): void { this.setSecurityThreats(_threats); }
