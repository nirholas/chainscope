...
import type { RegulationStance } from '@/config/crypto-regulation';
import { getCountriesGeoJson as getCountriesGeoJsonForRegulation } from '@/services/country-geometry';
...
export class DeckGLMap {
  ...
  private trackedFlightClusters: FlightCluster[] = [];
  private trackedVessels: TrackedVessel[] = [];
  private trackedVesselClusters: VesselCluster[] = [];
  ...
  private whaleActivityPoints: WhaleActivityPoint[] = [];
  ...
  constructor(container: HTMLElement, initialState: DeckMapState) {
    ...
    // Self-fetch TVL geographic data for heatmap
    this.fetchTvlGeo();

    // Self-fetch validator node distribution data
  ...
  private buildLayers(): LayersList {
    ...
    if (mapLayers.bases && this.isLayerVisible('bases')) {
      ...
      layers.push(this.createGhostLayer('bases-layer', TRACKED_FACILITIES, d => [d.lon, d.lat], { radiusMinPixels: 12 }));
    }
    ...
    if (mapLayers.nuclear && this.isLayerVisible('nuclear')) {
      ...
      layers.push(this.createGhostLayer('facility-layer', ENERGY_FACILITIES.filter(f => f.status !== 'decommissioned'), d => [d.lon, d.lat], { radiusMinPixels: 12 }));
    }
    ...
    // Security threat IOC layer
    if (mapLayers.cyberThreats && this.securityThreats.length > 0) {
      layers.push(this.createSecurityThreatsLayer());
      layers.push(this.createGhostLayer('cyber-threats-layer', this.securityThreats, d => [d.lon, d.lat], { radiusMinPixels: 12 }));
    }
    ...
    // Key ports layer (shown with AIS)
    if (mapLayers.ais) {
    ...
    // Tracked vessels layer
    if (mapLayers.military && this.trackedVessels.length > 0) {
      layers.push(this.createTrackedVesselsLayer());
    }

    // Tracked vessel clusters layer
    if (mapLayers.military && this.trackedVesselClusters.length > 0) {
      layers.push(this.createTrackedVesselClustersLayer());
    }

    // Tracked flights layer
    if (mapLayers.military && this.trackedFlights.length > 0) {
      layers.push(this.createTrackedFlightsLayer());
    }

    // Tracked flight clusters layer
    if (mapLayers.military && this.trackedFlightClusters.length > 0) {
      layers.push(this.createTrackedFlightClustersLayer());
    }
    ...
    // Exchange & Regulatory map layer (DeFi globe)
    if (mapLayers.exchangeMap) {
      const regLayer = this.createRegulatoryOverlayLayer();
      if (regLayer) layers.push(regLayer); // Country fills underneath
      layers.push(this.createExchangeMapLayer()); // Exchange dots on top
    }

    // Cross-chain bridge flow arcs (DeFi globe)
    if (mapLayers.chainFlows && this.bridgeFlows.length > 0) {
      layers.push(this.createChainFlowsLayer());
    }

    // Validator node distribution layer (DeFi globe)
    if (mapLayers.validatorNodes && this.validatorNodes.length > 0) {
      layers.push(this.createValidatorNodesLayer());
    }

    // Whale activity pulses (DeFi globe)
    if (mapLayers.whaleActivity && this.whaleActivityPoints.length > 0) {
      layers.push(this.createWhaleActivityLayer());
    }
  ...
  private createConflictZonesLayer(): GeoJsonLayer {
    const cacheKey = 'risk-zones-layer';

    const geojsonData = {
      type: 'FeatureCollection' as const,
      features: RISK_REGIONS.map(zone => ({
        type: 'Feature' as const,
  ...
  private createBasesLayer(): IconLayer {
    const highlightedBases = this.highlightedAssets.facility;
  ...
  private createNuclearLayer(): IconLayer {
    const highlightedNuclear = this.highlightedAssets.energy;
    const data = ENERGY_FACILITIES.filter(f => f.status !== 'decommissioned');
  ...
  private createSecurityThreatsLayer(): ScatterplotLayer<SecurityThreat> {
    return new ScatterplotLayer<SecurityThreat>({
      id: 'cyber-threats-layer',
      data: this.securityThreats,
      getPosition: (d) => [d.lon, d.lat],
      getRadius: (d) => {
        switch (d.severity) {
  ...
  private createTrackedVesselsLayer(): ScatterplotLayer {
    return new ScatterplotLayer({
      id: 'routes-layer',
      data: this.trackedVessels,
      getPosition: (d) => [d.lon, d.lat],
      getRadius: 6000,
  ...
  private createTrackedVesselClustersLayer(): ScatterplotLayer {
    return new ScatterplotLayer({
      id: 'route-clusters-layer',
      data: this.trackedVesselClusters,
      getPosition: (d) => [d.lon, d.lat],
  ...
  private createTrackedFlightsLayer(): ScatterplotLayer {
    return new ScatterplotLayer({
      id: 'trackers-layer',
      data: this.trackedFlights,
      getPosition: (d) => [d.lon, d.lat],
      getRadius: 8000,
  ...
  private createTrackedFlightClustersLayer(): ScatterplotLayer {
    return new ScatterplotLayer({
      id: 'tracker-clusters-layer',
      data: this.trackedFlightClusters,
      getPosition: (d) => [d.lon, d.lat],
  ...
  // === DeFi Globe Layers: Exchange & Regulatory Map ===

  /**
   * ScatterplotLayer showing major crypto exchange HQ locations.
   * Color-coded by regulatory status (licensed=green, restricted=yellow, banned=red, etc).
   */
  private createExchangeMapLayer(): ScatterplotLayer<CryptoExchange> {
    ...
    return new ScatterplotLayer<CryptoExchange>({
      id: 'exchange-map-layer',
      data: CRYPTO_EXCHANGES,
      getPosition: (d: CryptoExchange) => [d.lon, d.lat],
      getRadius: 20000,
      getFillColor: (d: CryptoExchange) => statusColors[d.status] || [150, 150, 150, 180],
      radiusMinPixels: 6,
      radiusMaxPixels: 18,
      pickable: true,
      stroked: true,
  ...
  /**
   * GeoJsonLayer overlay showing country regulatory stance for crypto.
   * Very subtle tinted fill — friendly=green, moderate=yellow, restrictive=orange, banned=red.
   * Uses the existing countries.geojson with async loading.
   */
  private regulatoryGeoJson: GeoJSON.FeatureCollection | null = null;
  private regulatoryGeoJsonLoading = false;

  private createRegulatoryOverlayLayer(): GeoJsonLayer | null {
    // Load GeoJSON asynchronously on first call
    if (!this.regulatoryGeoJson && !this.regulatoryGeoJsonLoading) {
      this.regulatoryGeoJsonLoading = true;
      getCountriesGeoJsonForRegulation().then(geojson => {
        if (geojson) {
          // Filter to only countries we have regulation data for
          this.regulatoryGeoJson = {
            type: 'FeatureCollection',
            features: geojson.features.filter(f => {
              const code = f.properties?.['ISO3166-1-Alpha-2'] || f.properties?.ISO_A2 || f.properties?.iso_a2;
              return code && REGULATION_BY_COUNTRY.has(code);
            }),
          };
          this.layerCache.delete('regulatory-overlay-layer');
          this.render();
        }
      });
      return null;
    }
    if (!this.regulatoryGeoJson) return null;

    return new GeoJsonLayer({
      id: 'regulatory-overlay-layer',
      data: this.regulatoryGeoJson,
      filled: true,
      stroked: false,
      getFillColor: (feature: GeoJSON.Feature) => {
        const code = feature.properties?.['ISO3166-1-Alpha-2'] || feature.properties?.ISO_A2 || feature.properties?.iso_a2;
        const reg = code ? REGULATION_BY_COUNTRY.get(code) : undefined;
        if (reg) return REGULATION_COLORS[reg.stance as RegulationStance];
        return [0, 0, 0, 0] as [number, number, number, number]; // transparent for unknown
      },
      pickable: false, // country fills are non-interactive; exchanges handle clicks
    });
  }
  ...
  private createProtestClusterLayers(): Layer[] {
    ...
    layers.push(this.createGhostLayer('event-clusters-layer', this.protestClusters, d => [d.lon, d.lat], { radiusMinPixels: 14 }));
  ...
  private createHotspotsLayers(): Layer[] {
    ...
    layers.push(this.createGhostLayer('trends-layer', this.hotspots, d => [d.lon, d.lat], { radiusMinPixels: 14 }));
  ...
  private getTooltip(info: PickingInfo): { html: string } | null {
    ...
    switch (layerId) {
      ...
      case 'earthquakes-layer':
        return { html: `<div class="deckgl-tooltip"><strong>M${(obj.magnitude || 0).toFixed(1)} Market Event</strong><br/>${text(obj.place)}</div>` };
      case 'routes-layer':
        return { html: `<div class="deckgl-tooltip"><strong>${text(obj.name)}</strong><br/>${text(obj.operatorCountry)}</div>` };
      case 'trackers-layer':
        return { html: `<div class="deckgl-tooltip"><strong>${text(obj.callsign || obj.registration || 'Whale Transaction')}</strong><br/>${text(obj.type)}</div>` };
      case 'route-clusters-layer':
      ...
      case 'event-clusters-layer':
        if (obj.count === 1) {
          ...
          return { html: `<div class="deckgl-tooltip"><strong>${text(item?.title || 'Governance Vote')}</strong><br/>${text(item?.city || item?.country || '')}</div>` };
        }
        return { html: `<div class="deckgl-tooltip"><strong>${obj.count} governance proposals</strong><br/>${text(obj.country)}</div>` };
      ...
      case 'cables-layer':
        return { html: `<div class="deckgl-tooltip"><strong>${text(obj.name)}</strong><br/>Cross-chain Bridge</div>` };
      case 'pipelines-layer':
        return { html: `<div class="deckgl-tooltip"><strong>${text(obj.name)}</strong><br/>${text(obj.type)} Yield Pipeline</div>` };
      case 'risk-zones-layer': {
        const props = obj.properties || obj;
        return { html: `<div class="deckgl-tooltip"><strong>${text(props.name)}</strong><br/>Exploit Zone</div>` };
      }
      ...
      case 'ais-density-layer':
        return { html: `<div class="deckgl-tooltip"><strong>DEX Volume</strong><br/>Intensity: ${text(obj.intensity)}</div>` };
      case 'waterways-layer':
        return { html: `<div class="deckgl-tooltip"><strong>${text(obj.name)}</strong><br/>Liquidity Pool</div>` };
      ...
      case 'nodes-layer':
        return { html: `<div class="deckgl-tooltip"><strong>${text(obj.name)}</strong><br/>${text(obj.type || 'MEV Bot')}</div>` };
      case 'spaceports-layer':
        return { html: `<div class="deckgl-tooltip"><strong>${text(obj.name)}</strong><br/>${text(obj.country || 'Launchpad')}</div>` };
      case 'ports-layer': {
      ...
      case 'ais-disruptions-layer':
        return { html: `<div class="deckgl-tooltip"><strong>Protocol ${text(obj.type || 'Disruption')}</strong><br/>${text(obj.severity)} severity<br/>${text(obj.description)}</div>` };
      ...
      case 'chain-flows-layer': {
        const vol = obj.volume24h >= 1e6 ? `$${(obj.volume24h / 1e6).toFixed(1)}M` : `$${(obj.volume24h / 1e3).toFixed(0)}K`;
        return { html: `<div class="deckgl-tooltip"><strong>${text(obj.sourceChain)} → ${text(obj.targetChain)}</strong><br/>${text(obj.bridgeName)}<br/>24h: ${vol}</div>` };
      }
      case 'validator-nodes-layer': {
        const pct = obj.percentage != null ? `${obj.percentage.toFixed(1)}%` : '';
        const clientInfo = obj.client ? Object.entries(obj.client).map(([k, v]) => `${k}: ${v}%`).join(', ') : '';
        return { html: `<div class="deckgl-tooltip"><strong>${text(obj.chain)} Nodes — ${text(obj.countryName)}</strong><br/>${obj.nodeCount.toLocaleString()} nodes (${pct})${clientInfo ? `<br/><small>${clientInfo}</small>` : ''}</div>` };
      }
      default:
        return null;
    }
  ...
  private handleClick(info: PickingInfo): void {
    ...
    // TVL city point click — show protocol breakdown
    if (layerId === 'tvl-cities-layer') {
      const point = info.object as TvlGeoPoint;
      const formatTvl = (v: number) => v >= 1e9 ? `$${(v / 1e9).toFixed(2)}B` : v >= 1e6 ? `$${(v / 1e6).toFixed(1)}M` : `$${(v / 1e3).toFixed(0)}K`;
      const protocolRows = point.protocols.slice(0, 15).map(p =>
        `<div style="display:flex;justify-content:space-between;padding:2px 0;font-size:11px;"><span>${escapeHtml(p.name)}</span><span style="color:#00ff64;font-weight:600;">${formatTvl(p.tvl)}</span></div>`
      ).join('');
      const moreCount = point.protocols.length > 15 ? point.protocols.length - 15 : 0;
      const html = `
        <div class="popup-header" style="background:linear-gradient(135deg,rgba(0,180,80,0.3),rgba(0,100,50,0.3));">
          <span class="popup-title">&#128178; ${escapeHtml(point.city)}, ${escapeHtml(point.country)}</span>
          <button class="popup-close">×</button>
        </div>
        <div class="popup-body">
          <div style="text-align:center;margin-bottom:8px;">
            <span style="font-size:18px;font-weight:700;color:#00ff64;">${formatTvl(point.tvl)}</span>
            <span style="font-size:11px;color:#aaa;margin-left:4px;">TVL</span>
          </div>
          <div style="font-size:11px;color:#888;margin-bottom:4px;text-transform:uppercase;">Protocols (${point.protocols.length})</div>
          ${protocolRows}
          ${moreCount > 0 ? `<div style="font-size:10px;color:#666;text-align:center;margin-top:4px;">+${moreCount} more</div>` : ''}
        </div>
      `;
      this.popup.showRaw(html, info.x, info.y);
      return;
    }

    // Validator node click — show chain + client breakdown
    if (layerId === 'validator-nodes-layer') {
      const node = info.object as ValidatorNode;
      const pct = node.percentage != null ? `${node.percentage.toFixed(1)}%` : '';
      const chainColors: Record<string, string> = {
        ethereum: '#627eea', solana: '#9945ff', polygon: '#8247e5',
        arbitrum: '#28a0f0', base: '#0052ff', optimism: '#ff0420',
      };
      const color = chainColors[node.chain] || '#64c8ff';
      const clientRows = node.client
        ? Object.entries(node.client)
            .sort((a, b) => (b[1] as number) - (a[1] as number))
            .map(([name, pctVal]) =>
              `<div style="display:flex;justify-content:space-between;padding:2px 0;font-size:11px;"><span>${escapeHtml(name)}</span><span style="color:${color};font-weight:600;">${pctVal}%</span></div>`
            ).join('')
        : '<div style="font-size:11px;color:#666;">Client data unavailable</div>';
      const html = `
        <div class="popup-header" style="background:linear-gradient(135deg,${color}44,${color}22);">
          <span class="popup-title">&#9878; ${escapeHtml(node.chain.charAt(0).toUpperCase() + node.chain.slice(1))} — ${escapeHtml(node.countryName)}</span>
          <button class="popup-close">×</button>
        </div>
        <div class="popup-body">
          <div style="text-align:center;margin-bottom:8px;">
            <span style="font-size:18px;font-weight:700;color:${color};">${node.nodeCount.toLocaleString()}</span>
            <span style="font-size:11px;color:#aaa;margin-left:4px;">nodes (${pct})</span>
          </div>
          <div style="font-size:11px;color:#888;margin-bottom:4px;text-transform:uppercase;">Client Distribution</div>
          ${clientRows}
        </div>
      `;
      this.popup.showRaw(html, info.x, info.y);
    ...
    const layerToPopupType: Record<string, PopupType> = {
      'risk-zones-layer': 'risk',
      'bases-layer': 'base',
      'facility-layer': 'nuclear',
      'nodes-layer': 'irradiator',
      ...
      'protests-layer': 'event',
      'trackers-layer': 'militaryFlight',
      'routes-layer': 'militaryVessel',
      'route-clusters-layer': 'militaryVesselCluster',
      'tracker-clusters-layer': 'militaryFlightCluster',
      ...
      'exchange-map-layer': 'exchange',
      'chain-flows-layer': 'bridgeFlow',
    };
    ...
    if (layerId === 'risk-zones-layer' && info.object.properties) {
      // Find the full risk zone data from config
      const conflictId = info.object.properties.id;
      const fullConflict = RISK_REGIONS.find(c => c.id === conflictId);
  ...
  private createLayerToggles(): void {
    ...
    const layerConfig = SITE_VARIANT === 'tech'
      ...
      : [
          { key: 'tvlHeatmap', label: 'TVL Heatmap', icon: '&#128178;' },
          { key: 'chainFlows', label: 'Chain Flows', icon: '&#127752;' },
          { key: 'validatorNodes', label: 'Validator Nodes', icon: '&#9878;' },
          { key: 'hotspots', label: 'DeFi Hotspots', icon: '&#127919;' },
          { key: 'conflicts', label: 'Exploit Zones', icon: '&#9876;' },
          { key: 'bases', label: 'Protocol HQs', icon: '&#127963;' },
          { key: 'nuclear', label: 'Blacklisted Wallets', icon: '&#9762;' },
          { key: 'irradiators', label: 'MEV Bots', icon: '&#9888;' },
          { key: 'spaceports', label: 'Launchpads', icon: '&#128640;' },
          { key: 'cables', label: 'Cross-chain Bridges', icon: '&#128268;' },
          { key: 'pipelines', label: 'Yield Pipelines', icon: '&#128738;' },
          { key: 'datacenters', label: 'Validator Nodes', icon: '&#128421;' },
          { key: 'military', label: 'Whale Activity', icon: '&#9992;' },
          { key: 'whaleActivity', label: 'Whale Pulses', icon: '&#128051;' },
          { key: 'ais', label: 'DEX Volume', icon: '&#128674;' },
          { key: 'flights', label: 'Gas Tracker', icon: '&#9992;' },
          { key: 'protests', label: 'Governance', icon: '&#128226;' },
          { key: 'ucdpEvents', label: 'Rug Pulls', icon: '&#9876;' },
          { key: 'displacement', label: 'Capital Flows', icon: '&#128101;' },
          ...
          { key: 'natural', label: 'Natural Events', icon: '&#127755;' },
          { key: 'fires', label: 'Fires', icon: '&#128293;' },
          { key: 'waterways', label: 'Liquidity Pools', icon: '&#9875;' },
          { key: 'economic', label: 'CEX Hubs', icon: '&#128176;' },
          { key: 'minerals', label: 'Token Launches', icon: '&#128142;' },
        ];
  ...
  private showLayerHelp(): void {
    ...
    const fullHelpContent = `
      ...
      <div class="layer-help-content">
        ...
        <div class="layer-help-section">
          <div class="layer-help-title">DeFi & Crypto</div>
          <div class="layer-help-item"><span>HOTSPOTS</span> Trending DeFi protocols and yield farms</div>
          <div class="layer-help-item"><span>EXPLOITS</span> Recent smart contract exploits and hack zones</div>
          <div class="layer-help-item"><span>SANCTIONS</span> OFAC-sanctioned addresses and mixer protocols</div>
          <div class="layer-help-item"><span>GOVERNANCE</span> Active DAO proposals and on-chain votes</div>
        </div>
        <div class="layer-help-section">
          <div class="layer-help-title">Protocol & Infrastructure</div>
          <div class="layer-help-item"><span>PROTOCOLS</span> Major DeFi protocol headquarters and teams</div>
          <div class="layer-help-item"><span>WALLETS</span> Flagged/blacklisted wallet clusters</div>
          <div class="layer-help-item"><span>MEV</span> MEV bot activity and sandwich attacks</div>
          <div class="layer-help-item"><span>WHALES</span> Large wallet movements and accumulation</div>
        </div>
        <div class="layer-help-section">
          <div class="layer-help-title">Infrastructure</div>
          <div class="layer-help-item"><span>BRIDGES</span> Cross-chain bridge protocols and TVL</div>
          <div class="layer-help-item"><span>YIELDS</span> Top yield farming pipelines and APY data</div>
          <div class="layer-help-item"><span>OUTAGES</span> RPC endpoint and network disruptions</div>
          <div class="layer-help-item"><span>VALIDATORS</span> Validator node geographic distribution</div>
        </div>
        <div class="layer-help-section">
          <div class="layer-help-title">Markets</div>
          <div class="layer-help-item"><span>DEX</span> Decentralized exchange volume heatmap</div>
          <div class="layer-help-item"><span>GAS</span> Network gas prices and congestion</div>
        </div>
        <div class="layer-help-section">
          <div class="layer-help-title">Data & Analytics</div>
          <div class="layer-help-item"><span>FLOWS</span> Capital flow between chains and protocols</div>
          <div class="layer-help-item"><span>WEATHER</span> Market sentiment and fear/greed indicators</div>
          <div class="layer-help-item"><span>CEX</span> Centralized exchange headquarters</div>
        </div>
        <div class="layer-help-section">
          <div class="layer-help-title">Labels</div>
          <div class="layer-help-item"><span>COUNTRIES</span> Country name overlays</div>
          <div class="layer-help-item"><span>POOLS</span> Major liquidity pool labels</div>
        </div>
      </div>
    `;
  ...
  private createValidatorNodesLayer(): ScatterplotLayer<ValidatorNode> {
    const CHAIN_COLORS: Record<string, [number, number, number, number]> = {
      ethereum: [98, 126, 234, 200],
      solana: [153, 69, 255, 200],
      polygon: [130, 71, 229, 200],
      arbitrum: [40, 160, 240, 200],
      base: [0, 82, 255, 200],
      optimism: [255, 4, 32, 200],
    };
    return new ScatterplotLayer<ValidatorNode>({
      ...
      getFillColor: (d: ValidatorNode) => CHAIN_COLORS[d.chain] || [100, 200, 255, 180],
      radiusMinPixels: 6,
      radiusMaxPixels: 40,
      pickable: true,
      stroked: true,
  ...
  public setValidatorNodes(nodes: ValidatorNode[]): void {
    this.validatorNodes = nodes;
    this.render();
  }

  // --- Whale Activity Pulses ---
  private createWhaleActivityLayer(): ScatterplotLayer<WhaleActivityPoint> {
    const now = Date.now();
    return new ScatterplotLayer<WhaleActivityPoint>({
      id: 'whale-activity-layer',
      data: this.whaleActivityPoints,
      getPosition: (d: WhaleActivityPoint) => [d.lon, d.lat],
      getRadius: (d: WhaleActivityPoint) => {
        const vol = d.inflowUSD + d.outflowUSD;
        // Scale: $1M → 20k, $100M → 200k, $1B → 500k
        return Math.min(500_000, Math.max(20_000, Math.sqrt(vol / 1_000_000) * 60_000));
      },
      getFillColor: (d: WhaleActivityPoint) => {
        const net = d.inflowUSD - d.outflowUSD;
        // Red for net inflow (bearish), green for net outflow (bullish)
        if (net > 0) return [255, 80, 80, 140] as [number, number, number, number];
        if (net < 0) return [80, 255, 120, 140] as [number, number, number, number];
        return [100, 180, 255, 120] as [number, number, number, number];
      },
      radiusMinPixels: 8,
      radiusMaxPixels: 60,
      pickable: true,
      stroked: true,
      getLineColor: (d: WhaleActivityPoint) => {
        const net = d.inflowUSD - d.outflowUSD;
        if (net > 0) return [255, 60, 60, 200] as [number, number, number, number];
        if (net < 0) return [60, 255, 100, 200] as [number, number, number, number];
        return [80, 160, 255, 180] as [number, number, number, number];
      },
      lineWidthMinPixels: 2,
      transitions: {
  ...
  public setWhaleActivity(points: WhaleActivityPoint[]): void {
    this.whaleActivityPoints = points;
    this.render();
  }

  // --- TVL Geographic Heatmap ---
  ...
  private createTvlHeatmapLayer(): HeatmapLayer<TvlGeoPoint> {
    return new HeatmapLayer<TvlGeoPoint>({
      id: 'tvl-heatmap-layer',
      data: this.tvlGeoPoints,
      getPosition: (d: TvlGeoPoint) => [d.lon, d.lat],
      getWeight: (d: TvlGeoPoint) => Math.log10(d.tvl + 1),
      radiusPixels: 60,
      intensity: 1.5,
      threshold: 0.05,
      colorRange: [
  ...
  private createTvlCitiesLayer(): ScatterplotLayer<TvlGeoPoint> {
    return new ScatterplotLayer<TvlGeoPoint>({
      id: 'tvl-cities-layer',
      data: this.tvlGeoPoints,
      getPosition: (d: TvlGeoPoint) => [d.lon, d.lat],
  ...
  public setTvlGeoPoints(points: TvlGeoPoint[]): void {
    this.tvlGeoPoints = points;
    this.render();
  }
  ...
  public setSecurityThreats(threats: SecurityThreat[]): void {
    this.securityThreats = threats;
    this.render();
  }
  /** @deprecated Use setSecurityThreats */
  public setCyberThreats(threats: SecurityThreat[]): void { this.setSecurityThreats(threats); }
  ...
  public setTrackedFlights(flights: TrackedFlight[], clusters: FlightCluster[] = []): void {
    this.trackedFlights = flights;
    this.trackedFlightClusters = clusters;
    this.render();
  }
  /** @deprecated Use setTrackedFlights */
  public setMilitaryFlights(flights: TrackedFlight[], clusters: FlightCluster[] = []): void { this.setTrackedFlights(flights, clusters); }

  public setTrackedVessels(vessels: TrackedVessel[], clusters: VesselCluster[] = []): void {
    this.trackedVessels = vessels;
    this.trackedVesselClusters = clusters;
    this.render();
  }
  /** @deprecated Use setTrackedVessels */
  public setMilitaryVessels(vessels: TrackedVessel[], clusters: VesselCluster[] = []): void { this.setTrackedVessels(vessels, clusters); }
  ...
  private createChainFlowsLayer(): ArcLayer<BridgeFlow> {
    return new ArcLayer<BridgeFlow>({
      ...
      getTargetColor: [0, 255, 100, 180],
      getWidth: (d: BridgeFlow) => Math.max(1, Math.log10(d.volume24h / 1e6 + 1) * 3),
      widthMinPixels: 1,
      widthMaxPixels: 8,
      pickable: true,
    });
  ...
  public setBridgeFlows(flows: BridgeFlow[]): void {
    this.bridgeFlows = flows;
    this.render();
  }
  ...
  public updateActivityForEscalation(flights: TrackedFlight[], vessels: TrackedVessel[]): void {
    setActivityData(flights, vessels);
  }
  /** @deprecated Use updateActivityForEscalation */
  public updateMilitaryForEscalation(flights: TrackedFlight[], vessels: TrackedVessel[]): void { this.updateActivityForEscalation(flights, vessels); }

  public getHotspotDynamicScore(hotspotId: string) {
    return getHotspotEscalation(hotspotId);
  }

  /** Get tracked flight clusters for rendering/analysis */
  public getTrackedFlightClusters(): FlightCluster[] {
    return this.trackedFlightClusters;
  }
  /** @deprecated Use getTrackedFlightClusters */
  public getMilitaryFlightClusters(): FlightCluster[] { return this.getTrackedFlightClusters(); }

  /** Get tracked vessel clusters for rendering/analysis */
  public getTrackedVesselClusters(): VesselCluster[] {
    return this.trackedVesselClusters;
  }
  /** @deprecated Use getTrackedVesselClusters */
  public getMilitaryVesselClusters(): VesselCluster[] { return this.getTrackedVesselClusters(); }
  ...
  public triggerConflictClick(id: string): void {
    const conflict = RISK_REGIONS.find(c => c.id === id);
    if (conflict) {
      ...
      const { x, y } = screenPos || this.getContainerCenter();
      this.popup.show({ type: 'risk', data: conflict, x, y });
    }
  }

  public triggerBaseClick(id: string): void {
    const base = TRACKED_FACILITIES.find(b => b.id === id);
    if (base) {
      // Don't pan - show popup at projected screen position or center
  ...
  public triggerNuclearClick(id: string): void {
    const facility = ENERGY_FACILITIES.find(n => n.id === id);
    if (facility) {
      // Don't pan - show popup at projected screen position or center
