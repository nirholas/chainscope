...
export class StablecoinPanel extends Panel {
  ...
  private async fetchData(): Promise<void> {
    ...
    try {
      const res = await fetch('/api/stablecoin-markets', { signal });
  ...
  private bindClicks(): void {
    const rows = this.content.querySelectorAll('.stable-row, .stable-supply-row');
    ...
    rows.forEach((row, i) => {
      ...
      row.addEventListener('click', () => {
        window.dispatchEvent(new CustomEvent('detail:open', {
          detail: { type: 'token', data: { symbol: c.symbol, name: c.name, price: c.price, marketCap: c.marketCap, volume24h: c.volume24h, change24h: c.change24h } }
        }));
      });
    });
  }
