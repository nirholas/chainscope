  ...
  constructor() {
    super({
      ...
      infoTooltip: `<strong>Methodology</strong>
        ...
        <ul>
          <li>50% Protocol Risk (top 5 weighted)</li>
          <li>30% On-chain convergence zones</li>
          <li>20% Smart contract incidents</li>
        </ul>
  ...
  private renderMetrics(): string {
    ...
    return `
      <div class="risk-metrics">
        ...
        <div class="risk-metric">
          <span class="risk-metric-value">${(this.overview.avgCIIDeviation ?? 0).toFixed(1)}</span>
          <span class="risk-metric-label">PRI Deviation</span>
        </div>
        <div class="risk-metric">
            ...
            if (isConvergence) {
              return `
                <div class="risk-item risk-item-clickable" data-lat="${topZone.lat}" data-lon="${topZone.lon}">
                  ...
                  <button class="detail-open-btn" data-detail-event='${escapeHtml(JSON.stringify({ title: risk, description: 'Top convergence zone', category: 'market-risk', lat: topZone.lat, lon: topZone.lon }))}' style="background:none;border:none;color:#555;cursor:pointer;font-size:14px;padding:2px 4px;opacity:0.6" title="View details">ℹ</button>
                </div>
              `;
            }
            return `
              <div class="risk-item">
                <span class="risk-rank">${i + 1}.</span>
                <span class="risk-text">${escapeHtml(risk)}</span>
                <button class="detail-open-btn" data-detail-event='${escapeHtml(JSON.stringify({ title: risk, category: 'market-risk' }))}' style="background:none;border:none;color:#555;cursor:pointer;font-size:14px;padding:2px 4px;opacity:0.6" title="View details">ℹ</button>
              </div>
            `;
          }).join('')}
        </div>
            ...
            return `
              <div class="risk-alert ${clickableClass}" style="border-left: 3px solid ${this.getPriorityColor(alert.priority)}" ${locationAttrs}>
                <div class="risk-alert-header">
                  ...
                  <button class="detail-open-btn" data-detail-event='${escapeHtml(JSON.stringify({ title: alert.title, description: alert.summary, category: alert.type, lat: alert.location?.lat, lon: alert.location?.lon }))}' style="background:none;border:none;color:#555;cursor:pointer;font-size:14px;padding:2px 4px;opacity:0.6" title="View details">ℹ</button>
                </div>
