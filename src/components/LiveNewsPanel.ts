...
export class LiveNewsPanel extends Panel {
  ...
  private get embedOrigin(): string {
    try { return new URL(getRemoteApiBaseUrl()).origin; } catch { return 'https://defistech.vercel.app'; }
  }
  ...
  private showOfflineMessage(channel: LiveChannel): void {
    ...
    `;
    this.content.querySelector('.offline-retry')?.addEventListener('click', () => {
      const activeBtn = this.element.querySelector('.live-channel-btn.active') as HTMLElement | null;
      activeBtn?.click();
    });
  }
