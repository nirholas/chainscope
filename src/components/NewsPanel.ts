  ...
  private renderFlat(items: NewsItem[]): void {
    ...
    const html = items
      ...
      <div class="item ${item.isAlert ? 'alert' : ''}" ${item.monitorColor ? `style="border-left-color: ${escapeHtml(item.monitorColor)}"` : ''}>
        ...
        <a class="item-title" href="${sanitizeUrl(item.link)}" target="_blank" rel="noopener">${escapeHtml(item.title)}</a><button class="detail-open-btn" data-detail-news='${detailJson}' style="background:none;border:none;color:#555;cursor:pointer;font-size:14px;padding:2px 4px;margin-left:4px;opacity:0.6" title="View details">ℹ</button>
  ...
  private renderClusterHtml(
    ...
    return `
      <div class="${itemClasses}" ${cluster.monitorColor ? `style="border-left-color: ${escapeHtml(cluster.monitorColor)}"` : ''} data-cluster-id="${escapeHtml(cluster.id)}" data-news-id="${escapeHtml(cluster.primaryLink)}">
        ...
        <a class="item-title" href="${sanitizeUrl(cluster.primaryLink)}" target="_blank" rel="noopener">${escapeHtml(cluster.primaryTitle)}</a><button class="detail-open-btn" data-detail-news='${escapeHtml(JSON.stringify({ title: cluster.primaryTitle, url: cluster.primaryLink, source: cluster.primarySource, publishedAt: cluster.lastUpdated, category: cluster.threat?.category }))}' style="background:none;border:none;color:#555;cursor:pointer;font-size:14px;padding:2px 4px;margin-left:4px;opacity:0.6" title="View details">ℹ</button>
