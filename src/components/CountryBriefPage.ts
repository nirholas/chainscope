...
export class CountryBriefPage {
  ...
  private static INFRA_LABELS: Record<BriefAssetType, string> = {
    pipeline: 'Yield Pipelines',
    cable: 'Cross-chain Bridges',
    datacenter: 'Validator Nodes',
    facility: 'Protocol HQs',
    energy: 'Flagged Wallets',
    port: 'DEX Hubs',
  };
  ...
  private componentBars(components: CountryScore['components']): string {
    const items = [
      { label: 'Usage', value: components.unrest, icon: '📢' },
      { label: 'Exploits', value: components.conflict, icon: '⚔' },
      { label: 'Security', value: components.security, icon: '🛡️' },
      { label: 'Sentiment', value: components.information, icon: '📡' },
    ];
  ...
  private signalChips(signals: CountryBriefSignals): string {
    ...
    if (signals.displacementOutflow > 0) {
      ...
      chips.push(`<span class="signal-chip displacement">🌊 ${fmt} outflows</span>`);
    }
    if (signals.climateStress > 0) chips.push(`<span class="signal-chip climate">🌡️ Climate stress</span>`);
    if (signals.conflictEvents > 0) chips.push(`<span class="signal-chip conflict">⚔️ ${signals.conflictEvents} exploit events</span>`);
            ...
            <div class="cb-col-left">
              ${score ? `
                <section class="cb-section cb-risk-section">
                  <h3 class="cb-section-title">Risk Index</h3>
                ...
                <section class="cb-section cb-risk-section">
                  ...
                  <div class="cb-not-tracked">
                    ...
                    <span>Not tracked — ${escapeHtml(country)} is not in the PRI tier-1 list</span>
                  </div>
                </section>`}

              <section class="cb-section cb-brief-section">
                <h3 class="cb-section-title">DeFi Brief</h3>
  ...
  public updateInfrastructure(countryCode: string): void {
    ...
    const assets = getNearbyInfrastructure(centroidLat, centroidLon, ['pipeline', 'cable', 'datacenter', 'facility', 'energy']);
    ...
    const order: BriefAssetType[] = ['pipeline', 'cable', 'datacenter', 'facility', 'energy', 'port'];
    let html = '';
  ...
  private exportBrief(format: 'json' | 'csv'): void {
    ...
    if (this.currentSignals) {
      data.signals = {
        ...
        trackedFlights: this.currentSignals.trackedFlights,
        trackedVessels: this.currentSignals.trackedVessels,
  ...
  public destroy(): void {
    ...
    if (this.boundExportMenuClose) {
      document.removeEventListener('click', this.boundExportMenuClose);
      this.boundExportMenuClose = null;
    }
    if (this.boundCitationClick) {
      document.removeEventListener('click', this.boundCitationClick);
