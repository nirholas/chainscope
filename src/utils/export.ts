...
export function exportToJSON(data: ExportData, filename = 'chainscope-export'): void {
  const jsonStr = JSON.stringify(data, null, 2);
  downloadFile(jsonStr, `${filename}.json`, 'application/json');
}

export function exportToCSV(data: ExportData, filename = 'chainscope-export'): void {
  const lines: string[] = [];
...
export class ExportPanel {
  ...
  constructor(getDataFn: () => ExportData) {
    ...
    this.boundDocumentClick = (e: MouseEvent) => {
      if (!this.element.contains(e.target as Node)) {
        this.isOpen = false;
        this.element.querySelector('.export-menu')?.classList.add('hidden');
      }
    };
  ...
  private export(format: ExportFormat): void {
    ...
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
    const filename = `chainscope-${timestamp}`;
