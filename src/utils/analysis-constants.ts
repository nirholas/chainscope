...
export const SIGNAL_CONTEXT: Record<SignalType, SignalContext> = {
  ...
  silent_divergence: {
    ...
    confidenceNote: 'Lower confidence as cause is unknown—treat as early warning, not confirmed signal.',
  },
  velocity_spike: {
  ...
  convergence: {
    ...
    actionableInsight: 'Treat this as high-confidence signal; triangulation reduces false positive risk.',
  ...
  triangulation: {
    ...
    actionableInsight: 'This is actionable signal data; expect market/policy reactions imminently.',
  ...
  hotspot_escalation: {
    whyItMatters: 'DeFi protocol showing significant risk escalation based on news activity, TVL changes, on-chain anomalies, and whale movements.',
    actionableInsight: 'Increase monitoring priority; assess downstream impacts on liquidity, yields, and protocol stability.',
    confidenceNote: 'Confidence weighted by multiple data sources—news (35%), protocol risk (25%), on-chain convergence (25%), whale activity (15%).',
  },
  sector_cascade: {
  ...
  military_surge: {
    whyItMatters: 'Whale wallet activity significantly above baseline—indicates potential accumulation, liquidation, or market manipulation.',
    actionableInsight: 'Correlate with protocol news; assess token price impact and liquidity depth.',
    confidenceNote: 'Higher confidence with sustained activity over multiple hours and diverse wallet addresses.',
  },
};
