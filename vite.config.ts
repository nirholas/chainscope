...
export default defineConfig({
  ...
  plugins: [
    ...
    VitePWA({
      ...
      manifest: {
        name: 'Chainscope - DeFi Operating System',
        short_name: 'Chainscope',
      ...
      workbox: {
        ...
        navigateFallbackDenylist: [/^\/api\//, /^\/settings/, /^\/intel/],
        skipWaiting: true,
        clientsClaim: true,
  ...
  server: {
    ...
    proxy: {
      ...
      // Catch-all for the ~100 handlers in api/*.js, which Vite cannot execute
      // itself. `npm run dev:api` runs them; without it these routes 404 and the
      // panels that depend on them show their error state rather than data.
      // Declared last so every specific rule above still wins.
      '/api': {
        target: `http://127.0.0.1:${process.env.LOCAL_API_PORT || 46123}`,
        changeOrigin: true,
        configure: (proxy) => {
          proxy.on('error', () => {
            console.log('[api] local handler server unreachable — run `npm run dev:api`');
          });
        },
      },
    },
