...
const KEYRING_SERVICE: &str = "chainscope";
/// Keyring namespace used by pre-rebrand builds. Reads fall back to it and
/// migrate the secret into KEYRING_SERVICE so saved API keys survive the rename.
...
const SUPPORTED_SECRET_KEYS: [&str; 16] = [
    "GROQ_API_KEY",
    "OPENROUTER_API_KEY",
...
/// Read a secret, falling back to the pre-rebrand keyring namespace. A hit in
/// the legacy namespace is copied into the current one and then removed, so
/// the migration happens at most once per key.
fn read_secret_migrating(key: &str) -> Result<Option<String>, String> {
    let entry = secret_entry(key)?;
    match entry.get_password() {
        Ok(value) => Ok(Some(value)),
        Err(keyring::Error::NoEntry) => {
            let Ok(legacy) = Entry::new(LEGACY_KEYRING_SERVICE, key) else {
                return Ok(None);
            };
...
fn build_app_menu(handle: &AppHandle) -> tauri::Result<Menu<tauri::Wry>> {
    ...
    let about_metadata = AboutMetadata {
        name: Some("Chainscope".into()),
        version: Some(env!("CARGO_PKG_VERSION").into()),
        copyright: Some("\u{00a9} 2026 Chainscope contributors".into()),
        website: Some("https://github.com/nirholas/chainscope".into()),
        website_label: Some("github.com/nirholas/chainscope".into()),
        ..Default::default()
    };
    let about_item = PredefinedMenuItem::about(handle, Some("About Chainscope"), Some(about_metadata))?;
...
fn handle_menu_event(app: &AppHandle, event: tauri::menu::MenuEvent) {
    match event.id().as_ref() {
        ...
        MENU_HELP_GITHUB_ID => {
            let _ = open_in_shell("https://github.com/nirholas/HQ");
        }
        MENU_HELP_DEVTOOLS_ID => {
...
fn start_local_api(app: &AppHandle) -> Result<(), String> {
    ...
    for key in SUPPORTED_SECRET_KEYS.iter() {
        if let Ok(Some(value)) = read_secret_migrating(key) {
...
fn main() {
    tauri::Builder::default()
        ...
        .expect("error while running Chainscope tauri application")
        .run(|app, event| {
            match &event {
