...
// Shares its allowlist with _cors.js (single source of truth) so the two
// don't drift — this one previously lacked the Chainscope production domains
// and the hq-*.vercel.app pattern that _cors.js already had.
function sanitizeOrigin(raw) {
  if (!raw) return 'https://defistech.vercel.app';
  try {
    const parsed = new URL(raw);
    if (parsed.protocol !== 'https:' && parsed.protocol !== 'http:' && parsed.protocol !== 'tauri:') {
      return 'https://defistech.vercel.app';
    }
