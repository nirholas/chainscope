...
export default async function handler(req) {
  ...
  try {
    const appId = btoa('chainscope:monitor@chainscope.local');
    ...
    // Optional overlay — degrade to an empty 200 so a transient HAPI outage
    // (the upstream occasionally rate-limits datacenter IPs) does not surface
    // as a console error on every page load.
    return Response.json({ success: false, degraded: true, error: `Fetch failed: ${toErrorMessage(error)}`, count: 0, countries: [] }, {
      status: 200,
