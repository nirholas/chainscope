...
function makeRequest(path = '/api/cyber-threats', ip = '198.51.100.10') {
  ...
  return new Request(`https://defis.tech${path}`, { headers });
}
...
test('API aggregates from all 5 sources', async () => {
  ...
  // This test exercises the handler without pinning nowMs, so mock activity
  // dates must be relative to the real clock or the days-window filter
  // silently drops every record as the suite ages.
